...
const NON_INTERACTIVE = !!process.env.ORQESTRA_INSTALL_MODE; // Env vars indicate non-interactive mode
...
export async function collectConfig(): Promise<InstallConfig> {
  ...
  const mode = NON_INTERACTIVE
    ? (process.env.ORQESTRA_INSTALL_MODE as InstallMode)
    : ((await select({
  ...
  bail(mode);
  if (NON_INTERACTIVE) log.ok(`Install mode: ${mode}`);
  ...
  const repoUrl = NON_INTERACTIVE
    ? (process.env.ORQESTRA_REPO_URL || DEFAULT_REPO)
    : ((await text({
        message: "Orqestra git repository",
        initialValue: process.env.ORQESTRA_REPO_URL ?? DEFAULT_REPO,
      })) as string);
  bail(repoUrl);
  if (NON_INTERACTIVE) log.ok(`Repository: ${repoUrl}`);

  const repoRef = NON_INTERACTIVE
    ? (process.env.ORQESTRA_REPO_REF || DEFAULT_REF)
    : ((await text({
        message: "Branch / tag",
        initialValue: process.env.ORQESTRA_REPO_REF ?? DEFAULT_REF,
      })) as string);
  bail(repoRef);
  if (NON_INTERACTIVE) log.ok(`Branch/tag: ${repoRef}`);
  ...
  if (isRoot()) {
    ...
    if (NON_INTERACTIVE) {
      sudo = "auto";
      log.ok("Non-interactive mode: defaulting to auto sudo.");
    } else {
  ...
  if (gpuPresent) {
    ...
    if (NON_INTERACTIVE) {
      gpu = process.env.ORQESTRA_GPU_MODE === "off" ? "off" : "auto";
      log.ok(`GPU support: ${gpu}`);
    } else {
  ...
  if (mode === "server") {
    ...
    if (NON_INTERACTIVE) {
      siteDomain = process.env.ORQESTRA_SITE_DOMAIN || "orqestra.local";
      letsEncryptEmail = process.env.ORQESTRA_LETS_ENCRYPT_EMAIL || "admin@orqestra.local";
      serverPublicIp = process.env.ORQESTRA_SERVER_PUBLIC_IP;
      if (!serverPublicIp) {
        const detectedIp = await detectPublicIp();
        serverPublicIp = detectedIp || "";
      }
      dns = process.env.ORQESTRA_CF_DNS_TOKEN ? "cloudflare-auto" : "manual";
      cfDnsApiToken = process.env.ORQESTRA_CF_DNS_TOKEN;
      log.ok(`Domain: ${siteDomain}`);
      log.ok(`DNS mode: ${dns}`);
    } else {
  ...
  if (NON_INTERACTIVE) {
    ...
    if (anthropicApiKey) {
      log.ok("AI diagnostics enabled via ANTHROPIC_API_KEY");
    } else {
      log.info("AI diagnostics disabled (not set)");
    }
  } else {
  ...
  // ----- Generate secrets -------------------------------------------------
  const postgresPassword = process.env.ORQESTRA_POSTGRES_PASSWORD || (await randomHex(16));
  const betterAuthSecret = process.env.ORQESTRA_BETTER_AUTH_SECRET || (await randomHex(32));
  const internalApiSecret = process.env.ORQESTRA_INTERNAL_API_SECRET || (await randomHex(32));
